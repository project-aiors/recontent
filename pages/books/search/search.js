...
function searchCSV(searchValue) {
  ...
  const categorizeMatch = (row) => {
    ...
    const tags = (typeof row.author === "string" ? row.author : "").toLowerCase();
...
function displaySearchResults(results) {
  ...
  results.forEach((row) => {
    ...
    // Try loading real cover
    if (row.coverImg) {
      const testImg = new Image();
      testImg.onload = () => {
        posterImg.src = row.coverImg;
        posterImg.style.filter = ""; // remove invert if needed
      };
      testImg.src = row.coverImg;
    }
    ...
    dateDiv.textContent = row.publishDate;
    ...
    resultItem.addEventListener('click', () => {
      ...
      localStorage.setItem('chosenContent', JSON.stringify([row.bookId, row.title, row.language, row.publishDate,row.coverImg]));
...
function displaySearchedResults(results) {
  ...
  results.forEach((row) => {
    ...
    // Create an image element for the movie poster
    const posterImg = document.createElement("img");
    posterImg.className = "searched-results-poster";


    const detailsDiv = document.createElement("div");
    detailsDiv.textContent = row.publishDate + " | " + row.language + " | ⭐" + row.rating;
